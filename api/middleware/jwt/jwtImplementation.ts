...
export function isTokenValid(request) {
  if (request.cookies.authorization) {
    const decoded = jwt.verify(
      request.cookies.authorization1.toString(),
      jwtSecret.key
    );
    if (decoded) {
      return true;
...
export function createTokens(response, results) {
  const token = jwt.sign({ userId: results[0].ID_Usuario }, jwtSecret.key, {
    expiresIn: "1h",
  });
  ...
  response
    .cookie("authorization", [token], { secure: true, httpOnly: true })
...
export function refreshToken(req, res) {
  if (!req.cookies.refreshToken) {
    ...
    const decoded = jwt.verify(
      req.cookies.refreshToken.toString(),
      jwtSecret.key
    );
    const token = jwt.sign({ userId: decoded.userId }, jwtSecret.key, {
      expiresIn: "1h",
    });
